...
public abstract class WTFTest implements IHookable {
	 ...
	 * default browser.
	 */
	public static Browser getCurrentBrowser(ITestContext context,
			ITestResult testResult) {
		Browser browserToRun = null;

		// From test groups.
		for (String groupName : testResult.getMethod().getGroups()) {
			if (groupName.equals("BROWSER")) {
				browserToRun = Browser.getBrowserNameFromList(testResult
						.getMethod().getGroups());
				break;
			}
		}
		...
		if (browserToRun == null
			...
			List<Browser> enabledBrowserList = new ArrayList<Browser>(
					Arrays.asList(TestBrowserAnnotationReader
							.getIncludedBrowsers(testResult.getMethod()
									.getMethod())));
			if (enabledBrowserList.size() > 0
					&& Collections.binarySearch(enabledBrowserList,
							browserToRun) < 0) {
				browserToRun = Browser.HTML_UNIT;
			}
		}

		// Load FF as default browser.
		if (browserToRun == null) {
			browserToRun = Browser.FIREFOX;
		}
		return browserToRun;
	}
	 ...
	 * default browser.
	 */
	public Country getCurrentSite(ITestContext context, ITestResult testResult) {
		Country countryToRun = null;

		// Load it from the XML suite test parameter.
		if (countryToRun == null) {
			countryToRun = getParameter(context, "site") != null ? Country
		...
		if (countryToRun == null && WTFTestConfig.getSites().length == 1
				&& WTFTestConfig.getSites()[0] != Country.US) {
	...
	public void captureScreenShoot(WTFEnv env, ITestResult testResult) {
		if (!env.getBrowser().equals(Browser.NATIVE_HTML_UNIT))
			testResult.setAttribute("TEST_HTML_SOURCE", env.getDriver()
					.getPageSource());
		if (env.getBrowser().equals(Browser.HTML_UNIT)) {
			LOG(Level.INFO,
					String.format("Screenshots Not Applicable For HTML UNIT Tests."));
		}
		...
		// }
		if (!env.getBrowser().equals(Browser.HTML_UNIT)
				&& !env.getBrowser().equals(Browser.NATIVE_HTML_UNIT)
...
/*			WTFDashScreenshotUtil.setTestScreenShotForWTFDash(screenshotFile,
	...
	public void run(final IHookCallBack icb, ITestResult testResult) {
		...
		Browser browser = getCurrentBrowser(envWithContext.getTestContext(),
				testResult);

		Country country = getCurrentSite(envWithContext.getTestContext(),
				testResult);
		...
		LOG(Level.INFO, String.format(
				"Launching Driver: Browser=%s, Site=%s, Environment=%s",
				browser.getBrowserName().toUpperCase(), country, environment));

		if (!(browser == Browser.NATIVE_HTML_UNIT)) {
	 ...
	 * profile.
	 */
	protected FirefoxProfile loadFirefoxProfile(Country country) {
	...
	private DesiredCapabilities getBasicBrowserCapabilities(Browser browser) {
		switch (browser) {
		case FIREFOX:
	...
	private DesiredCapabilities getAdvancedBrowserCapabilities(Browser browser) {
		return new DesiredCapabilities(browser.browserNameOnGrid,
				browser.version, browser.os != null ? browser.getPlatform()
						: Platform.WINDOWS);
	}

	protected DesiredCapabilities getDesiredCapabilities(Browser browser) {
		return browser.version != null ? getAdvancedBrowserCapabilities(browser)
				: getBasicBrowserCapabilities(browser);
	}

	public synchronized WebDriver getDriver(Browser browser,
			ITestResult testResult) {
		return getDriver(browser, null, testResult);
	}

	public synchronized WebDriver getDriver(Browser browser, Country country,
			ITestResult testResult) {
		switch (browser) {
		case FIREFOX:
	 ...
	 */
	public WebDriver getDriverWithRetry(Browser browser, Country country,
			ITestResult testResult) {
		...
		while (reTryMax > 0) {
			...
			try {
				if (WTFTestConfig.getRemoteWebDriverHubURL() != null) {
					...
					if (browser.equals(Browser.HTML_UNIT)) {
	 ...
	 * Returns a new webdriver instance for the given browser type.
	 */
	public WebDriver getDriverWithRetryFromBackupGrid(Browser browser,
			ITestResult testResult) {
		String testName = testResult.getName();
		WebDriver driver = null;
		int reTryMax = 3;
