...
public class BaseListener implements IReporter {
  ...
  public static String getTestMethodNameinPackageStyle(ITestResult result) {
    ...
    logTextPart = logTextPart.replace("class com.ebay.webdriver.", "..");
    return logTextPart;
  }
  ...
  public static String getBrowserName(ITestResult result) {
    Browser browser = WTFTest.getCurrentBrowser(result.getTestContext(), result);
    return browser.toString();
    
//    result.getTestContext()
//    return getIRA(result).browser.toString();
  }
  ...
  public void printStatus(int status, ITestResult result) {
    ...
    case 16:
      ...
      LOG(Level.INFO, String.format("[STARTING %s/%s] [%s] [%s]%s", counter, totalTestCount, browser, site, logTextPart));
      break;
    case 32:
